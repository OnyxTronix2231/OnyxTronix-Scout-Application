   ...
   public class SortedList<T> : ICollection<T> where T : IComparable<T>
{
    ...
    {
        int insertIndex = FindIndexForSortedInsert(innerList, comparer, item);
    ...
    {
        int insertIndex = FindIndexForSortedInsert(innerList, comparer, item);
        if (insertIndex == innerList.Count)
        {
            return -1;
        }
        if (comparer.Compare(item, innerList[insertIndex]) == 0)
        {
            int index = insertIndex;
            while (index > 0 && comparer.Compare(item, innerList[index - 1]) == 0)
            {
                index--;
            }
            return index;
