...
namespace OnyxScoutApplication.Shared.Models.FluentValidations
{
    public class ScoutFormForamtValidator : AbstractValidator<ScoutFormFormatDto>
    {
        public ScoutFormForamtValidator()
        {
            ...
            RuleFor(x => x.AutonomousFields.Concat(x.TeleoperatedFields).Concat(x.EndGameFields)).Must(i => !i.GroupBy(f => f.Name).Where(x => x.Skip(1).Any()).Any())
                .WithMessage("Fields name must be unique");
        }
    }
}
